...
def _write(path,title,rows,base_url):
    fg=FeedGenerator(); fg.title(title); fg.link(href=base_url); fg.description(title)
    for p in sorted(rows,key=lambda x:(x.score,x.issued_date),reverse=True)[:200]:
...
def write_all_feeds(dir,permits,base_url):
    ...
    _write(dir/'top-opportunities.xml','Idaho Top Opportunities',[p for p in q if p.score>=35],base_url)
